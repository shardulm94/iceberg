...
public abstract class OrcSchemaWithTypeVisitor<T> {
  ...
  public static <T> T visit(Type iType, TypeDescription schema, OrcSchemaWithTypeVisitor<T> visitor) {
    switch (schema.getCategory()) {
      case STRUCT:
        return visitRecord(iType == null? null: iType.asStructType(), schema, visitor);
      ...
      case LIST:
        Types.ListType list = iType == null? null: iType.asListType();
        return visitor.array(
            list, schema,
            visit(list.elementType(), schema.getChildren().get(0), visitor));

      case MAP:
        Types.MapType map = iType == null? null: iType.asMapType();
        return visitor.map(
            map, schema,
      ...
      default:
        return visitor.primitive(iType == null? null: iType.asPrimitiveType(), schema);
    }
  }
